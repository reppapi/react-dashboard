...
function App() {
  ...
  const fetchStatus = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setMqttPing(data.mqtt_ping ?? 12);
        setBattery(data.battery ?? 100);
        setOptimizationMode(Boolean(data.optimization_mode));
        setBuzzerActive(Boolean(data.buzzer_active));
        setActivity(data.activity_prediction || data.activity || "Awake");
        setSedentaryMinutes(data.sedentary_minutes ?? 0);
        setLdrValue(data.ldr_value ?? 200);
      } else {
  ...
  const handleDismissAlert = async () => {
    try {
      const response = await fetch("http://localhost:5000/api/dismiss", {
        method: "POST",
      });
      if (response.ok) {
        fetchStatus();
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-[#f2f6fc] via-[#edf3fa] to-[#e4eef9] text-on-surface">
      ...
      <div className="flex-1 flex flex-col min-h-screen md:ml-[260px] pb-20 md:pb-0">
        ...
        <main className="flex-1 p-4 md:p-10 max-w-[1440px] mx-auto w-full">
          ...
          {activePage === "health" && (
            <HealthMetrics cardsVisible={cardsVisible} />
          )}
          {activePage === "history" && (
            <ActivityHistory cardsVisible={cardsVisible} />
          )}
          {activePage === "devices" && (
            <Devices
              cardsVisible={cardsVisible}
